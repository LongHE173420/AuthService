...
function login(usernameRaw, password, deviceId) {
    ...
    if (acc.password !== password)
        return (0, response_1.fail)("Sai mật khẩu");
    ...
    if (needOtp) {
        ...
        return (0, response_1.ok)({ needOtp: true, otpSample: otp }, `Cần OTP. OTP mẫu: ${otp}`);
    }
    const tokens = (0, token_1.issueTokens)(username, true);
    return (0, response_1.ok)({ needOtp: false, tokens }, "Đăng nhập thành công");
}
function verifyLoginOtp(usernameRaw, otpRaw, deviceId) {
    ...
    if (!pending)
        return (0, response_1.fail)("Không có phiên OTP login");
    if (pending.session.expiresAt <= (0, time_1.now)()) {
        db_1.db.pendingLogin.delete(key);
        return (0, response_1.fail)("Phiên OTP đã hết hạn");
    }
    ...
    db_1.db.pendingLogin.delete(key);
    const tokens = (0, token_1.issueTokens)(username, true);
    return (0, response_1.ok)({ tokens }, "Xác thực OTP login thành công");
}
function resendLoginOtp(usernameRaw, deviceId) {
    ...
    return (0, response_1.ok)({ otpSample: otp, phase: (0, otpSession_1.getPhase)(pending.session) }, `Đã gửi lại OTP. OTP mẫu: ${otp}`);
}
function refresh(refreshToken) {
    if ((0, token_1.isRefreshExpired)(refreshToken)) {
        // TH2 refresh hết hạn -> client clear + login lại
        return (0, response_1.fail)("REFRESH_TOKEN_EXPIRED");
    }
    const username = refreshToken.split(".")[1]?.toLowerCase?.() ?? "";
    if (!username)
        return (0, response_1.fail)("Refresh token không hợp lệ");
    const acc = db_1.db.accounts.get(username);
    if (!acc)
        return (0, response_1.fail)("Tài khoản không tồn tại");
    // TH3 refresh fail do revoked
    if (acc.revoked)
        return (0, response_1.fail)("REVOKED_DEVICE_OTHER_LOGIN");
    const tokens = (0, token_1.issueTokens)(username, true);
    return (0, response_1.ok)({ tokens }, "Refresh token thành công");
}
...
function debugRevoke(usernameRaw) {
    ...
    if (!acc)
        return (0, response_1.fail)("Tài khoản không tồn tại");
    acc.revoked = true;
    return (0, response_1.ok)(null, "Đã revoke (refresh sẽ fail)");
}
